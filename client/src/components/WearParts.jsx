  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-stone-300">Common parts to watch</h3>
      ...
      {parts.length === 0 ? (
        loaded && !loading && <p className="text-sm text-stone-500">No list generated yet.</p>
      ) : (
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h4 className="font-medium text-stone-100 text-sm">{part.part}</h4>
                    {lastService ? (
                  ...
                  {part.symptoms && <p className="text-xs text-stone-500 mt-0.5">Symptoms: {part.symptoms}</p>}
                  {part.notes && <p className="text-xs text-stone-600 mt-0.5 italic">{part.notes}</p>}
                </div>
              </div>
            );
          })}
